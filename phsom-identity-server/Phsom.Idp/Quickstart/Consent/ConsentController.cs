...
namespace IdentityServer4.Quickstart.UI
{
    ...
    public class ConsentController : Controller
    {
        ...
        public async Task<IActionResult> Index(ConsentInputModel model)
        {
            ...
            {
                if (await clientStore.IsPkceClientAsync(result.ClientId).ConfigureAwait(false))
                {
        ...
        private async Task<ProcessConsentResult> ProcessConsent(ConsentInputModel model)
        {
            ...
            var request = await interaction.GetAuthorizationContextAsync(model.ReturnUrl).ConfigureAwait(false);
            if (request == null)
            {
                return result;
            }
            ...
            if (model.Button == "no")
            {
                ...
                await events.RaiseAsync(
                    new ConsentDeniedEvent(User.GetSubjectId(), result.ClientId, request.ScopesRequested)).ConfigureAwait(false);
            }
                ...
                if (model.ScopesConsented != null && model.ScopesConsented.Any())
                {
                    ...
                    await events.RaiseAsync(
                        new ConsentGrantedEvent(
                            ...
                            grantedConsent.RememberConsent)).ConfigureAwait(false);
                }
                else
                {
            ...
            if (grantedConsent != null)
            {
                ...
                await interaction.GrantConsentAsync(request, grantedConsent).ConfigureAwait(false);
        ...
        {
            var request = await interaction.GetAuthorizationContextAsync(returnUrl).ConfigureAwait(false);
            if (request != null)
            {
