...
namespace IdentityServer4.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            // if the TestUserStore is not in DI, then we'll just use the global userManager collection
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await events.RaiseAsync(new UserLoginSuccessEvent(user.UserName, user.Id.ToString(), user.UserName)).ConfigureAwait(false);
                ...
                await events.RaiseAsync(new UserLoginFailureEvent(model.Username, "invalid credentials")).ConfigureAwait(false);
